...
int main() 
{
    ...
    std::string s1{"abcd"},s2,s3;
    std::copy(s1.begin(),s1.end(), std::back_inserter(s2)); // (K)
    cout << "12|    copy s1='" << s1 << "', back_inserter s2='" << s2 << "'" << endl;
    std::list<char> l2;
    std::copy(s1.begin(),s1.end(), std::front_inserter(l2));
    std::copy(l2.begin(),l2.end(), std::back_inserter(s3));
    cout << "12|    copy, s1='" << s1 << "', front_inserter l2.front='" << l2.front()
         << "', back_inserter s3='" << s3 << "'" << endl;
 ...
 * (J)  ... oder rotieren.
 *
 * (K)  Je nach STL-Container ist front- oder back-inserter verfügbar und
 *      generiert push_front- oder push_back-Aufrufe. Damit ist das Kopieren
 *      eines ganzen Containers oder eines Teils leicht möglich.
 * 
 */
